...
- **流式输出** — AI 回复像打字一样逐字显示，不用等完整结果，体验和 ChatGPT 网页版一致，随时可点 ⏹ 停止生成（已生成的部分会保留）
- **💭 思考过程展示** — 使用 deepseek-reasoner 等推理模型时，思考过程会实时显示在可折叠的灰色区域里，不再是漫长的空白等待
...
- **参数调节** — 每个模型配置可单独设置温度（temperature）和最大回复长度（max_tokens），留空则用服务商默认值

### 用户体验
- **🎨 主题色定制** — 蓝 / 紫 / 绿 / 橙四种预设，还有个取色器可以自己调任意颜色
- **🌙 深色模式** — 晚上不刺眼，一键切换
- **📱 / 🖥 双布局** — 紧凑模式像手机 App，宽屏模式像 ChatGPT 网页版（左侧对话列表 + 右侧聊天区 + 右侧设置面板）
- **📋 Markdown 渲染** — AI 回复里的代码会高亮着色并带一键复制按钮，表格、列表、引用、链接都能正确显示
